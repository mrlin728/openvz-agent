require('../src/runtime/console-redaction.cjs')
...
function resolvePortableRoot() {
  ...
  const requestedRoot = (process.env.OPENVZ_PORTABLE_DIR || process.env.BAILONGMA_PORTABLE_DIR)?.trim()
...
if (PORTABLE_USER_DIR) {
  ...
  process.env.OPENVZ_USER_DIR ||= PORTABLE_USER_DIR
  process.env.BAILONGMA_USER_DIR ||= process.env.OPENVZ_USER_DIR
}
...
const STARTUP_STEPS = [
  ...
  { id: 'core', label: '启动本地核心', detail: '加载 OpenVZ Agent runtime' },
...
global.openvzStartupProgress = emitStartupProgress
...
// 持久化日志：把 console.* 镜像到 USER_DIR/logs/openvz-agent.log，
// 安装版没有 stdout 的情况下，卡死/崩溃后还能 tail 这个文件复盘。
// 简易 rotate：> 5MB 时把当前文件改名 .old（覆盖上一份 .old），下次写入重开。
const LOG_DIR = path.join(USER_DIR, 'logs')
const LOG_FILE = path.join(LOG_DIR, 'openvz-agent.log')
const LOG_FILE_OLD = path.join(LOG_DIR, 'openvz-agent.old.log')
...
global.getOpenVZWindowLayoutSnapshot = getBailongmaWindowLayoutSnapshot
const appControl = {
  restart() {
...
global.openvzAppControl = appControl
global.bailongmaAppControl = appControl
...
function validatePackagedNativeModules() {
  ...
  const nativePath = path.join(moduleRoot, 'prebuilds', `${PLATFORM}-${process.arch}.node`)
  const issues = []
  ...
  if (issues.length) {
    throw new Error(`Packaged native module integrity check failed:\n${issues.join('\n')}\nPlease close OpenVZ Agent and reinstall it with the official installer.`)
  }
}

async function bootstrapBackend(port) {
  process.env.OPENVZ_USER_DIR ||= USER_DIR
  process.env.OPENVZ_RESOURCES_DIR ||= RESOURCE_ROOT
  process.env.OPENVZ_PORT = String(port)
  process.env.BAILONGMA_USER_DIR ||= process.env.OPENVZ_USER_DIR
  process.env.BAILONGMA_RESOURCES_DIR ||= process.env.OPENVZ_RESOURCES_DIR
  process.env.BAILONGMA_PORT ||= process.env.OPENVZ_PORT
...
function createTerminalStreamWindow(payload = {}) {
  const { title = 'OpenVZ Agent Terminal Stream', stream_id = 'default' } = payload
  const cleanTitle = String(title || 'OpenVZ Agent Terminal Stream').slice(0, 120)
...
let runtimeShutdownBeforeQuit = null
let runtimeShutdownComplete = false
app.on('before-quit', (event) => {
  app.isQuiting = true
  if (runtimeShutdownComplete) return
  const shutdownHandlers = [
    globalThis.shutdownOpenVZBrowserTools || globalThis.shutdownBailongmaBrowserTools,
    globalThis.shutdownOpenVZMcp,
  ...
  event.preventDefault()
  if (runtimeShutdownBeforeQuit) return
  // The backend runs in this process. Wait for Playwright and MCP cleanup, with
  // a hard upper bound so a broken child process cannot trap the quit loop.
  let runtimeShutdownTimer
  runtimeShutdownBeforeQuit = Promise.race([
    Promise.allSettled(shutdownHandlers.map(handler => Promise.resolve().then(() => handler()))),
    new Promise(resolve => { runtimeShutdownTimer = setTimeout(resolve, 5_000) }),
  ]).finally(() => {
    clearTimeout(runtimeShutdownTimer)
    runtimeShutdownComplete = true
    app.quit() // second before-quit observes runtimeShutdownComplete and proceeds
  })
})
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error('[main] Failed to load OpenVZ Agent UI', err?.stack || err?.message || err)
    emitStartupProgress({ id: 'interface', status: 'error', error: true, message: `进入界面失败: ${err.message}` })
    dialog.showErrorBox('Startup failed', `Unable to load the OpenVZ Agent interface:\n${err.message}`)
    app.quit()
    return
  }
  setupTray()
