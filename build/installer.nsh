...
Function BailongmaNormalizeInstallDir
  ...
  ; Always install into an application-owned child folder. A folder named
  ; Bailongma is retained only when it contains a recognized 2.1.x payload, so
  ; upgrades do not nest OpenVZ Agent inside the historical install directory.
  ${GetFileName} "$INSTDIR" $R0
  StrCmp $R0 "OpenVZ Agent" openvzNormalizeDone
  StrCmp $R0 "Bailongma" 0 openvzNormalizeAppend
  IfFileExists "$INSTDIR\Bailongma.exe" openvzNormalizeDone
  IfFileExists "$INSTDIR\OpenVZ Agent.exe" openvzNormalizeDone
  IfFileExists "$INSTDIR\resources\app.asar" openvzNormalizeDone
  openvzNormalizeAppend:
    StrCpy $INSTDIR "$INSTDIR\OpenVZ Agent"
  openvzNormalizeDone:
FunctionEnd

Function BailongmaFindForeignInstallRootItem
  ; Returns the first item in $INSTDIR that is not owned by OpenVZ Agent.
  ...
  bailongmaScanInstallRoot:
    ...
    StrCmp $R2 ".." bailongmaScanInstallRootNext
    StrCmp $R2 "OpenVZ Agent.exe" bailongmaScanInstallRootNext
    StrCmp $R2 "Uninstall OpenVZ Agent.exe" bailongmaScanInstallRootNext
...
Function BailongmaInstallDirSafetyPageCreate
  ...
  ${NSD_CreateLabel} 0 0 100% 24u "OpenVZ Agent will be installed into this application-owned folder:"
  Pop $R1
  ${NSD_CreateText} 0 28u 100% 14u "$INSTDIR"
  Pop $R2
  EnableWindow $R2 0
  ${NSD_CreateLabel} 0 52u 100% 48u "If you chose D:\ or D:\Apps, the installer automatically adds the OpenVZ Agent subfolder. Program files stay here; conversations, memories, settings, API keys, sandbox files, and downloads stay under %APPDATA%\OpenVZ Agent and are removed only if you explicitly choose to clear user data during uninstall."
  Pop $R3
  nsDialogs::Show
FunctionEnd
...
Function BailongmaValidateInstalledPayload
  ...
  IfFileExists "$INSTDIR\resources\app.asar.unpacked\node_modules\better-sqlite3\prebuilds\win32-x64.node" 0 bailongmaPayloadMissing

  ClearErrors
  FileOpen $R1 "$INSTDIR\OpenVZ Agent.exe" r
  ...
  ; Use a conservative lower bound (50 MB) that any complete OpenVZ Agent.exe far
...
!macro customInstall
  ...
  SetOutPath "$INSTDIR"
  Delete "$SMPROGRAMS\OpenVZ Agent.lnk"
  Delete "$SMPROGRAMS\OpenVZ Agent\OpenVZ Agent.lnk"
  RMDir "$SMPROGRAMS\OpenVZ Agent"
  Delete "$DESKTOP\OpenVZ Agent.lnk"
  ...
  CreateShortCut "$DESKTOP\OpenVZ Agent.lnk" "$INSTDIR\OpenVZ Agent.exe" "" "$INSTDIR\OpenVZ Agent.exe" 0
  WriteRegStr SHELL_CONTEXT "${INSTALL_REGISTRY_KEY}" "KeepShortcuts" "true"
  ${if} $installMode == "all"
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "UninstallString" '"$INSTDIR\Uninstall OpenVZ Agent.exe" /allusers --keep-shortcuts'
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "QuietUninstallString" '"$INSTDIR\Uninstall OpenVZ Agent.exe" /allusers /S --keep-shortcuts'
  ${else}
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "UninstallString" '"$INSTDIR\Uninstall OpenVZ Agent.exe" /currentuser --keep-shortcuts'
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "QuietUninstallString" '"$INSTDIR\Uninstall OpenVZ Agent.exe" /currentuser /S --keep-shortcuts'
  ${endIf}
!macroend
...
!macro customInit
  ...
  ${if} $R0 != "OpenVZ Agent"
    ${if} $R0 == "Bailongma"
      ...
      ${orIf} ${FileExists} "$INSTDIR\OpenVZ Agent.exe"
      ${orIf} ${FileExists} "$INSTDIR\resources\app.asar"
        DetailPrint "Recognized legacy Bailongma install directory; upgrading in place to OpenVZ Agent."
      ${else}
        Call BailongmaNormalizeInstallDir
      ${endIf}
    ${else}
      ${if} ${FileExists} "$INSTDIR\OpenVZ Agent.exe"
      ${orIf} ${FileExists} "$INSTDIR\Bailongma.exe"
  ...
  ; Even a folder named OpenVZ Agent can contain user-created or third-party
...
!macro customRemoveFiles
  ...
  Delete "$INSTDIR\OpenVZ Agent.exe"
  ...
  Delete "$INSTDIR\Uninstall OpenVZ Agent.exe"
  ...
  RMDir /r "$INSTDIR\resources\app.asar.unpacked"
  RMDir /r "$INSTDIR\resources\playwright-browsers"
...
!macro customUnInstall
  ...
  ${ifNot} ${isUpdated}
    MessageBox MB_YESNO|MB_ICONQUESTION "是否同时删除 OpenVZ Agent 的全部用户数据？$\r$\n$\r$\n包括：对话与记忆数据库、配置（含 API Key）、沙盒文件、下载的音乐等。$\r$\n$\r$\n选择「是」将彻底清除且无法恢复；选择「否」保留数据，方便以后重装时继续使用。" /SD IDNO IDNO keepUserData
      ; userData 目录 = %APPDATA%\<productName>，即 $APPDATA\OpenVZ Agent
      RMDir /r "$APPDATA\OpenVZ Agent"
    keepUserData:
  ${endIf}
!macroend
