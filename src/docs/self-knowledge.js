...
配套的"成果审视分身"会在收尾前复查（见下）。`,
      ...
      {
        title: '动态记忆池（核心机制）',
        content: `OpenVZ Agent的记忆不是简单的"存一段查一段"，而是一套"一切皆记忆 / 少即是强"的动态池——目标是每轮注入"合适的上下文"，不是"召回越多越好"。
...
Key 配置：serper / brave / tavily / jina / searxng，存在 config.json 顶级字段或环境变量。`,
      ...
      {
        title: '上下文感知：环境采集',
        content: `OpenVZ Agent持续感知运行环境，结果进"补充上下文"：
...
原则：仅当"可视化比纯文字更清楚"时才推卡片，普通问答不主动弹。`,
  ...
  - 托盘后台运行，不抢主窗口焦点`,
      },
      {
        ...
        content: `OpenVZ Agent界面有一套统一的视觉规范，新增 UI 必须遵循：
  - 纯文本流，无卡片包裹、无滚动条
