...
import { handleWorkflowRoutes } from './api/routes/workflows.js'
import {
...
function getApiHost() {
  const envHost = String(globalThis.process?.env?.OPENVZ_HOST || globalThis.process?.env?.BAILONGMA_HOST || '').trim()
...
function isLanAccessEnabled() {
  return getNetworkConfig().allowLanAccess
    || /^(1|true|yes|on)$/i.test(String(globalThis.process?.env?.OPENVZ_ALLOW_LAN || globalThis.process?.env?.BAILONGMA_ALLOW_LAN || '').trim())
}
...
function getAuthToken() {
  return String(globalThis.process?.env?.OPENVZ_API_TOKEN || globalThis.process?.env?.BAILONGMA_API_TOKEN || '').trim()
}
...
async function dispatchHttpRoutes(req, res, url, context) {
  ...
  if (await handleWorkflowRoutes(req, res, url)) return true
