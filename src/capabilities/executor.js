...
import { executeMcpTool, getMcpToolSchema, isMcpTool, listMcpTools } from '../mcp/index.js'
...
import { execListWorkflows, execPlanWorkflow, execReplayWorkflow, execRunWorkflow } from './tools/workflow.js'
import { execLegacyUIHide, execLegacyUIPatch, execLegacyUIRegister, execLegacyUIShow, execLegacyUIUpdate } from './tools/acui-compat.js'
...
function getDesktopWindowLayoutSnapshot() {
  try {
    const reader = globalThis?.getOpenVZWindowLayoutSnapshot || globalThis?.getBailongmaWindowLayoutSnapshot
...
async function executeToolUnchecked(name, args, context = {}) {
  try {
    ...
    switch (name) {
      ...
      case 'ui_show':
        return execLegacyUIShow(args)
      case 'ui_hide':
        return execLegacyUIHide(args)
      case 'ui_update':
        return execLegacyUIUpdate(args)
      case 'ui_patch':
        return execLegacyUIPatch(args)
      case 'ui_register':
        return execLegacyUIRegister(args)
...
function execFindTool({ query } = {}) {
  ...
  const catalog = [
    ...
    ...listMcpTools().map(t => ({ name: t.name, description: t.description || '' })),
  ]
...
function execTerminalStream({
  ...
  text = '',
  stream_id = 'default',
  title = 'OpenVZ Agent Terminal Stream',
  newline = true,
  level = 'info',
  format = '',
  artifact_kind = '',
  ...
  const cleanTitle = String(title || 'OpenVZ Agent Terminal Stream').trim() || 'OpenVZ Agent Terminal Stream'
...
function execGrantAgentDelegation({ allowed, note = '' }) {
  ...
  const msg = allowed
    ? `已记录授权：OpenVZ Agent 可以指挥本地 AI 小伙伴工作。`
