...
export function getPrimaryChromiumLaunchOptions(env = process.env) {
  ...
  const channel = String(env.OPENVZ_BROWSER_CHANNEL || env.BAILONGMA_BROWSER_CHANNEL || 'chrome').trim().toLowerCase()
  if (!['chrome', 'chromium', 'msedge'].includes(channel)) {
    throw new Error(`Unsupported OPENVZ_BROWSER_CHANNEL: ${channel}`)
  }
