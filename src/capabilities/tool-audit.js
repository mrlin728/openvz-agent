...
const SECRET_VALUE_RE = /\b(?:sk|ak|ark|rk|pk|ghp|github_pat|xox[abprs])-[-A-Za-z0-9_.]{10,180}\b/gi
...
function safeResultPreview(name, result, error) {
  if (name !== 'browser_act') {
    ...
    try {
      return previewValue(redactAuditValue(JSON.parse(String(value))), 220)
    } catch {
      return previewValue(redactAuditText(value), 220)
    }
  }
  try {
...
export function buildToolAuditRecord({ name, args, context, policy, status, result = '', error = '', startedAt }) {
  ...
  return {
      ...
      resultPreview,
      error: name === 'browser_act' && error ? 'browser action failed' : redactAuditText(error),
      durationMs,
