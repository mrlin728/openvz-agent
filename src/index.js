import './runtime/console-redaction.cjs'
...
import { sanitizeToolAuditArgs } from './capabilities/tool-audit.js'
...
function reportStartupProgress(id, status, detail, message) {
  try {
    const reporter = globalThis.openvzStartupProgress || globalThis.bailongmaStartupProgress
...
await withStartupTimeout(loadMcpServers(), 12000, '[startup] mcp-servers')
  ...
  try {
    ...
    llmResult = await callLLM({
      ...
      onToolCall: (name, args, result) => {
        ...
        const eventArgs = sanitizeToolAuditArgs(name, cleanArgs)
        ...
        emitEvent('tool_call', { name, args: eventArgs, result: resultForEvent, ok })
        const recognizerResultLimit = ok ? 500 : 1200
        toolCallLog.push({ name, args: eventArgs, result: resultText.slice(0, recognizerResultLimit), ok, fallback: isFallbackDelivery, ack: isAckDelivery })
...
async function main() {
  ...
  const apiPort = Number(process.env.OPENVZ_PORT || process.env.BAILONGMA_PORT) || 3721
