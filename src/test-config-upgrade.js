...
const { decryptSecretsDeep } = await import('./secure-store.js')
...
function readSecureJson(file) {
  return decryptSecretsDeep(JSON.parse(fs.readFileSync(file, 'utf-8')))
}
...
// ── 场景 E：schema 迁移 v0 → v3，旧版 seedance、LLM 和 voice 块拆到独立文件 ──
{
  ...
  const sd = readSecureJson(seedanceFile)
  ...
  const llm = readSecureJson(llmFile)
  assert(llm.apiKey === 'sk-deepseek-valid-key-1234567890' && llm.model === 'deepseek-v4-pro', 'E: LLM provider 文件数据完整')
  const voiceActive = readSecureJson(path.join(voiceDir, 'active.json'))
  const voiceAliyun = readSecureJson(path.join(voiceDir, 'aliyun.json'))
...
// Scenario J: provider files preserve old keys and allow switching back without re-entering a key.
{
  ...
  const deepseekCfg = readSecureJson(deepseekFile)
