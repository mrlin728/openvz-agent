...
function errorInfo(errorOrMessage) {
  if (errorOrMessage instanceof Error) {
    return {
      ...
      message: truncateString(redactSecretText(errorOrMessage.message || '')),
      stack: redactSecretText(errorOrMessage.stack || `${errorOrMessage.name || 'Error'}: ${errorOrMessage.message || ''}`),
    }
  }
  const message = truncateString(redactSecretText(errorOrMessage ?? 'Unknown error'))
  return {
    name: 'Error',
    message,
    stack: message,
