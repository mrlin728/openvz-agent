...
function testErrorTextSecretRedaction() {
  const secret = 'sk-error-log-fixture-1234567890'
  const { result: payload, calls } = captureConsole(() => logError(
    new Error(`provider rejected api_key=${secret} Authorization: Bearer ${secret}`),
    { scope: 'security', operation: 'redaction' },
  ))
