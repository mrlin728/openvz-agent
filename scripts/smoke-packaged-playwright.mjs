...
const exe = path.join(unpacked, 'OpenVZ Agent.exe')
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'openvz-packaged-playwright-'))
...
const expectedChromium = path.resolve(process.env.OPENVZ_EXPECTED_CHROMIUM)
...
process.env.OPENVZ_BROWSER_CHANNEL = 'chromium'
...
async function runProbe() {
  const child = spawn(exe, [probeFile], {
    ...
    env: {
      ...
      OPENVZ_EXPECTED_CHROMIUM: chromiumExe,
...
function packagedChromiumPids() {
  const result = spawnSync('powershell.exe', [
    '-NoProfile', '-NonInteractive', '-Command',
    "$expected=[IO.Path]::GetFullPath($env:OPENVZ_EXPECTED_CHROMIUM); Get-CimInstance Win32_Process -Filter \"Name='chrome.exe'\" | Where-Object { $_.ExecutablePath -and [IO.Path]::GetFullPath($_.ExecutablePath) -ieq $expected } | Select-Object -ExpandProperty ProcessId",
  ], {
    encoding: 'utf8',
    timeout: 15_000,
    windowsHide: true,
    env: { ...process.env, OPENVZ_EXPECTED_CHROMIUM: chromiumExe },
  })
...
try {
  ...
  try {
    ...
    // Check from the parent after OpenVZ Agent.exe has exited. A detached browser
  ...
  const installers = fs.readdirSync(path.join(root, 'dist'))
    .filter(name => /^OpenVZ-Agent-Setup\.exe$/i.test(name))
