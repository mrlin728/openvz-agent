...
assert.match(release, /v21439-packaged-fixture\.py create/, 'Windows release smoke must create a realistic legacy user-data fixture before native rebuild')
assert.match(smoke, /v21439-packaged-fixture\.py'\) verify/, 'Windows smoke must independently verify the packaged migration')
assert.doesNotMatch(smoke, /-Label 'silent upgrade'/, 'Windows smoke must not treat a same-version repair as an upgrade')
assert.match(release, /Fresh install and packaged v2\.1\.439 data migration smoke\s+timeout-minutes: 20/, 'Windows smoke step must have a workflow timeout')
