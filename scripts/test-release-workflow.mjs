...
assert.match(smoke, /Start-Job -ScriptBlock/, 'installer operations must be wrapped so the whole process tree can be timed out')
assert.match(smoke, /Start-Process -FilePath \$TargetPath -ArgumentList \$targetArguments -Wait -PassThru/, 'installer operations must wait for the complete NSIS process tree')
assert.match(smoke, /Wait-Job -Job \$job -Timeout \$TimeoutSeconds/, 'installer process-tree waits must have an explicit timeout')
assert.doesNotMatch(smoke, /WaitForExit\(\$TimeoutSeconds \* 1000\)/, 'installer smoke must not wait for only the NSIS bootstrapper')
assert.match(release, /Download and verify v2\.1\.439 upgrade baseline[\s\S]+47f875643abdfd1c8b679a12090ca6376b07158f994ff97db772341a6764a388/, 'Windows upgrades must use the pinned published baseline')
assert.match(smoke, /v2\.1\.439 to current-version silent upgrade/, 'Windows smoke must exercise a real old-to-new upgrade')
assert.doesNotMatch(smoke, /-Label 'silent upgrade'/, 'Windows smoke must not treat a same-version repair as an upgrade')
assert.match(release, /Fresh install and v2\.1\.439 upgrade smoke\s+timeout-minutes: 20/, 'Windows smoke step must have a workflow timeout')
