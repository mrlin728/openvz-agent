...
assert.doesNotMatch(html, /v2\.2\.0-rc\.[123456]/, 'download page must not retain failed candidate tags')
...
for (const [kind, asset] of [
  ...
  assert.match(html, new RegExp(`data-download="${kind}"[^>]+releases/download/v2\\.2\\.0-rc\\.7/${asset.replaceAll('.', '\\.')}`))
}
